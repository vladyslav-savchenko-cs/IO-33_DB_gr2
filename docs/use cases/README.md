...
| **ID:**                | **DeleteOwnPost** |
|:-----------------------|:---------------|
| **НАЗВА:**             | Видалення власонго поста |
| **УЧАСНИКИ:**          | Користувач, Система |
...
@startuml
    |Адміністратор|
    start;
    : Заходить в панель модерації;
    : Вибирає пост, який потрібно видалити;

    |Система|
    : Перевіряє, що користувач має роль "адміністратор";
    note right #FFB3B3
    <b> Можлива
    <b> "NotAuthenticated"
    end note
    ...
    |Адміністратор|
    : Надсилає запит DELETE /api/post/:id;
    ...
    : Видаляє пост із бази даних;

    |Адміністратор|
    : Отримує повідомлення про успішне видалення поста;
    stop;
...
@startuml
    ...
    : Вводить дані нового користувача;

    |Система|
    : Перевіряє, чи існує користувач із таким ім'ям;
    note right #FFB3B3
    <b> Можлива
    <b> "UsernameExists"
    end note
    ...
    |Адміністратор|
    : Отримує повідомлення про успішне створення користувача;
    stop;
